...
if __name__ == '__main__':
    ...
    X_test=np.concatenate((X_train_11[:6000],X_train_12[:6000],X_train_13[:6000]),axis=0)
    #test_l1,test_l2,test_l_cqi=layer_split(X_test)
    y_test=np.concatenate((y_train_11[:6000],y_train_12[:6000],y_train_13[:6000]),axis=0)
    ...
    for tl in range(1):
        ...
        loss=float('inf')
        model_type = ms.jf_mapes
        #exec('oname="njf_cl_coteach_weight_mape_allmetric_alg_%s"%tl')
        oname='njf_cl_coteach_mloss_mape_allmetric_alg'
        ...
        for ii in range(ne):
            if (ii-ip)<160:
                ...
                record_metrics2.append([md,mse,mv,cr,aa,ab,ac])
                ...
                for idx in range(10):
                    ...
                    w = np.maximum(1-w,0)
                    model_B.fit(temp_train,temp_label, epochs=1, batch_size=256, verbose=0,sample_weight=w)
                    
                    predicts = model_B.predict(temp_train).flatten()
                    w = np.abs((predicts-temp_label)/temp_label)
                    w = np.maximum(1-w,0)
                    model_A.fit(temp_train,temp_label, epochs=1, batch_size=256, verbose=0,sample_weight=w)
        ...
        #model_A.save(path_model, overwrite=True)
    ...
    model_A = load_model(path_model_A,custom_objects={'m_loss2': ms.m_loss2})
    model_B = load_model(path_model_B,custom_objects={'m_loss2': ms.m_loss2})
    predicts = (model_A.predict(X_test).flatten()+model_B.predict(X_test).flatten())/2
    #predicts = model.predict([test_l1,test_l2,test_l_cqi])
