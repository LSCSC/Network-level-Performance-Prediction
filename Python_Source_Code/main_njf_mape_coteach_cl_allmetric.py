...
if __name__ == '__main__':
    ...
    for tl in range(1):
        print('iteration: %d'%tl)
        record_metrics2=list()
        loss=float('inf')
        model_type = ms.jf_mapev
        #exec('oname="njf_cl_coteach_mape_allmetric_alg_%s"%tl')
        oname='njf_cl_coteach_mape_allmetric_alg'
        model_name = oname+'.h5'
        ...
        np.random.seed(83)
        model_A.fit(X_train,y_train, epochs=1, batch_size=64, verbose=1)
        model_B.fit(X_train,y_train, epochs=1, batch_size=64, verbose=1)
